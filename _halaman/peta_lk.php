...
?> <!-- Judul Halaman -->
 ...
 <?=content_open('Peta Desa Berdasarkan Luas Lahan Kering')?><!-- Judul Konten -->
                ...
                var legend = L.control({position: 'bottomleft'});
                      legend.onAdd = function (map) {
                        ...
                        var div = L.DomUtil.create('div', 'info legend'),
                          grades = [0, 100, 200,300, 500, 700, 900],
                          labels = ['Luas Wilayah (ha)<br>'],
                          from, to;
    ...
    while ($d = mysqli_fetch_array($data)) {
      ?>
          var myStyle<?=$d['id_desa']?>={
              ...
              "dashArray": '3',
              "fillOpacity": 0.7,
              "fillColor": "<?=$d['warna_lk']?>"
          }
      <?php
      $arraydes[]='{
        name: "'.str_replace('_', ' ', $d['nama_desa']).'",
        icon: iconByName("'.$d['warna'].'"),
        layer:  new L.GeoJSON.AJAX(["assets/geojson/'.$d['nama_desa'].'.geojson"] ,{onEachFeature:onEachFeature,style:myStyle'.$d['id_desa'].',pointToLayer: featureToMarker }).addTo(map).bindPopup("<b>Desa '.$d['nama_desa'].' </b><br>Luas Wilayah: '.$d['luas'].' ha<br>Luas Pemukiman : '.$d['bangunan'].' ha<br>Luas Kebun : '.$d['kebun'].' ha<br>Luas Hutan Negara: '.$d['hutan_negara'].' ha")
      }';
    }
      # code...
